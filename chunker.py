"""
chunker.py — Chunking syntaxique pour Python (ast) et C++ (tree-sitter)
...
def chunk_file(path: Path, root: Path) -> List[CodeChunk]:
    ...
    Dispatch vers PythonChunker ou CppChunker selon l'extension.
    Retourne [] si le fichier n'est pas supporté ou en cas d'erreur.
    """
    ext = path.suffix.lower()
    try:
        print(f"Chunking : {path.relative_to(root)}...")
